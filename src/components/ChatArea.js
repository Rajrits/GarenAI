...
const ChatArea = () => {
    ...
    function formatGardenData(data) {
        let formattedText = '';
        ...
        data.split('* ').forEach((item, index) => {
          if (index !== 0) {
            ...
            const plants = parts[1].split(', ').join('\n- ');
            formattedText += `**${title}**:\n- ${plants}\n\n`;
          }
        });
    ...
    const fetchData = async (userText) => {
        ...
        await axios.get(`http://localhost:5000/api/getdata?question=${userText}`)
            .then((response) => { data = response.data.Bot;})
